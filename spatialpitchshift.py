...
class SPATIALPITCHSHIFT(MIDITokenizer):
    """ MIDI encoding method, similar to the REMI introduced in the Pop Music Transformer paper
    https://arxiv.org/abs/2002.00212
    ...
    def track_to_tokens(self, track: Instrument) -> List[int]:
        ...
        previous_tick_events = [[],[]]
        current_tick_events = [[],[]]

        for note in track.notes:
            init = 0
            if note.start != previous_tick: # Nouveau tas de notes

                if current_tick_events == [[],[]]: # Initialize current list (*)

                    if previous_tick_events == [[],[]]: # Initialize previous list

                        # Update previous_tick_events

                        # (Rest)
                        if (self.additional_tokens['Rest'] and note.start > previous_note_end and note.start - previous_note_end >= min_rest):
                            ...
                            if rest_beat > 0:
                                previous_tick_events[1].append(Event(type_='Rest', time=previous_note_end, value=f'{rest_beat}.0',
                                                    desc=f'{rest_beat}.0'))
                            ...
                            while rest_pos >= self.rests[0][1]:
                                ...
                                previous_tick_events[1].append(Event(type_='Rest', time=previous_note_end, value=f'0.{rest_pos_temp}',
                        ...
                        for i in range(nb_new_bars):
                            previous_tick_events[1].append(Event(type_='Bar', time=(current_bar + i + 1) * ticks_per_bar, value=None, desc=0))
                        current_bar += nb_new_bars

                        # Position
                        pos_index = int((note.start % ticks_per_bar) / ticks_per_sample)
                        previous_tick_events[1].append(Event(type_='Position', time=note.start, value=pos_index, desc=note.start))
                        ...
                        if self.additional_tokens['Tempo']:
                            ...
                            previous_tick_events[1].append(Event(type_='Tempo', time=note.start, value=current_tempo, desc=note.start))
                        
                        # Pitch / Velocity / Duration
                        previous_tick_events[0].append(Event(type_='Pitch', time=note.start, value=note.pitch, desc=note.pitch))
                        previous_tick_events[0].append(Event(type_='Velocity', time=note.start, value=note.velocity, desc=f'{note.velocity}'))
                        duration = note.end - note.start
                        index = np.argmin(np.abs(dur_bins - duration))
                        previous_tick_events[0].append(Event(type_='Duration', time=note.start, value='.'.join(map(str, self.durations[index])),
                        ...
                        if (self.additional_tokens['Rest'] and note.start > previous_note_end and note.start - previous_note_end >= min_rest):
                            ...
                            if rest_beat > 0:
                                current_tick_events[1].append(Event(type_='Rest', time=previous_note_end, value=f'{rest_beat}.0',
                                                    desc=f'{rest_beat}.0'))
                                previous_tick += rest_beat * self.current_midi_metadata['time_division']

                            while rest_pos >= self.rests[0][1]:
                                rest_pos_temp = min([r[1] for r in self.rests], key=lambda x: abs(x - rest_pos))
                                current_tick_events[1].append(Event(type_='Rest', time=previous_note_end, value=f'0.{rest_pos_temp}',
                                                    desc=f'0.{rest_pos_temp}'))
                                previous_tick += round(rest_pos_temp * ticks_per_sample)
                                rest_pos -= rest_pos_temp
                        ...
                        nb_new_bars = note.start // ticks_per_bar - current_bar
                        for i in range(nb_new_bars):
                            current_tick_events[1].append(Event(type_='Bar', time=(current_bar + i + 1) * ticks_per_bar, value=None, desc=0))
                        current_bar += nb_new_bars

                        # Position
                        pos_index = int((note.start % ticks_per_bar) / ticks_per_sample)
                        current_tick_events[1].append(Event(type_='Position', time=note.start, value=pos_index, desc=note.start))
                            ...
                            if current_tempo_idx + 1 < len(self.current_midi_metadata['tempo_changes']):
                                ...
                                for tempo_change in self.current_midi_metadata['tempo_changes'][current_tempo_idx + 1:]:
                                    ...
                                    if tempo_change.time <= note.start:
                                        ...
                                        break  # this tempo change is beyond the current time step, we break the loop
                            current_tick_events[1].append(Event(type_='Tempo', time=note.start, value=current_tempo, desc=note.start))
                        
                        # Pitch / Velocity / Duration
                        current_tick_events[0].append(Event(type_='Pitch', time=note.start, value=note.pitch, desc=note.pitch))
                        current_tick_events[0].append(Event(type_='Velocity', time=note.start, value=note.velocity, desc=f'{note.velocity}'))
                        duration = note.end - note.start
                        index = np.argmin(np.abs(dur_bins - duration))
                        current_tick_events[0].append(Event(type_='Duration', time=note.start, value='.'.join(map(str, self.durations[index])),
                                            desc=f'{duration} ticks'))
                    ...
                    # Now is time to create pitchshifts

                    previous_pitchshifts = []
                    current_pitchshifts = []
                    res = []

                    for event in previous_tick_events[1]:
                        res.append(event)

                    highest_pitch_previous_tick = -1
                    high_pitch = -1
                    for event in previous_tick_events[0][::-1]:
                        if event.type == 'Pitch':
                            if highest_pitch_previous_tick == -1: # First we find the highest note of this tick
                                highest_pitch_previous_tick = event.value
                            else:
                                if high_pitch == -1: # Then we initialize the pitchshift
                                    high_pitch = event.value
                                    previous_pitchshifts.append(high_pitch - highest_pitch_previous_tick)
                                else:
                                    previous_pitchshifts.append(event.value - high_pitch) # to create a list of pitchshifts values
                                    high_pitch = event.value

                    highest_pitch_current_tick = -1
                    high_pitch = -1
                    for event in current_tick_events[0][::-1]:
                        if event.type == 'Pitch':
                            if highest_pitch_current_tick == -1: # First we find the highest note of this tick
                                highest_pitch_current_tick = event.value

                    horizontal_pitchshift = highest_pitch_current_tick - highest_pitch_previous_tick
                    res.append(Event(type_='HPitchshift', time=event.time, value=horizontal_pitchshift, desc=horizontal_pitchshift))

                    if previous_pitchshifts != []:
                        index = -1
                        for event in previous_tick_events[0]:
                            if event.type == 'Pitch':
                                if index == -1:
                                    index = 0 # Skips the first pitch
                                else: # create vpitchshift event
                                    res.append(Event(type_='VPitchshift', time=event.time, value=previous_pitchshifts[::-1][index], desc=previous_pitchshifts[::-1][index]))
                                    index += 1
                            else:
                                res.append(event) # keep the tokens that are not Pitch
                    else:
                        for event in previous_tick_events[0]:
                            if event.type != 'Pitch':
                                res.append(event) # keep the tokens that are not Pitch

                    events.extend(res) # Update events with spatial pitchshift

                    # Loop
                
                    previous_tick_events = current_tick_events
                    current_tick_events = [[],[]]
            ...
            if init != 1 and current_tick_events == [[],[]]: # Fully create previous list
                # Update previous_tick_events
            ...
            if init != 1 and current_tick_events != [[],[]]: # Fully create current list
                # Update current_tick_events
        ...
        # print to ckeck our tokens
        #table_data = [['TOKEN TYPE', 'TIME', 'VALUE', 'DESCRIPTION']]
        #for event in events:
        #    table_data.append([str(event.type), str(event.time), str(event.value), str(event.desc)])
