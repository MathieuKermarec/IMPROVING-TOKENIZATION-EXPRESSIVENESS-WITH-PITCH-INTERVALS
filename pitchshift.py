...
class PITCHSHIFT(MIDITokenizer):
    """ MIDI encoding method, similar to the REMI introduced in the Pop Music Transformer paper
    https://arxiv.org/abs/2002.00212
    ...
    def track_to_tokens(self, track: Instrument) -> List[int]:
        ...
        # Events are now created and sorted as a REMI token Sequence
        # We go through this sequence to add pitchshifts and remove pitches
        
        ps_events = [] # New list of events to convert to tokens
        previous_pitch = -1
        current_pitch = 0

        for event in events:
            if event.type == 'Pitch':
                if previous_pitch == -1: # if this is the first note of the track
                    previous_pitch = event.value # initialize
                else:
                    current_pitch = event.value 
                    current_pitchshift = current_pitch - previous_pitch # compute the pitchshift
                    ps_events.append(Event(type_='Pitchshift', time=event.time, value=current_pitchshift, desc=current_pitchshift))
                    previous_pitch = current_pitch # iterate

            else:
                ps_events.append(event) # keep the tokens that are not Pitch
        ...
        return self.events_to_tokens(ps_events)
    ...
    def _create_vocabulary(self, sos_eos_tokens: bool = None) -> Vocabulary:
        ...
        # PITCHSHIFT
        ps_index = 0
        vocab.add_event(f'Pitchshift_{ps_index}')
